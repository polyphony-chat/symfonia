    ...
    fn try_from(message: tokio_tungstenite::tungstenite::Message) -> Result<Self, Self::Error> {
        ...
        match dispatch_event_type {
            ...
            DispatchEventType::CallCreate => {
                return convert_to!(DispatchEvent::CallCreate, message_as_string)
                    .map(Event::Dispatch)
            }
            DispatchEventType::CallUpdate => {
                return convert_to!(DispatchEvent::CallUpdate, message_as_string)
                    .map(Event::Dispatch)
            }
            DispatchEventType::CallDelete => {
                return convert_to!(DispatchEvent::CallDelete, message_as_string)
                    .map(Event::Dispatch)
            }
            ...
            DispatchEventType::ThreadCreate => {
                return convert_to!(DispatchEvent::ThreadCreate, message_as_string)
                    .map(Event::Dispatch)
            }
            DispatchEventType::ThreadUpdate => {
                return convert_to!(DispatchEvent::ThreadUpdate, message_as_string)
                    .map(Event::Dispatch)
            }
            DispatchEventType::ThreadDelete => {
                return convert_to!(DispatchEvent::ThreadDelete, message_as_string)
                    .map(Event::Dispatch)
            }
            ...
            DispatchEventType::GuildCreate => {
                return convert_to!(DispatchEvent::GuildCreate, message_as_string)
                    .map(Event::Dispatch)
            }
            DispatchEventType::GuildUpdate => {
                return convert_to!(DispatchEvent::GuildUpdate, message_as_string)
                    .map(Event::Dispatch)
            }
            DispatchEventType::GuildDelete => {
                return convert_to!(DispatchEvent::GuildDelete, message_as_string)
                    .map(Event::Dispatch)
            }
            ...
            DispatchEventType::GuildBanAdd => {
                return convert_to!(DispatchEvent::GuildBanAdd, message_as_string)
                    .map(Event::Dispatch)
            }
            ...
            DispatchEventType::InviteCreate => {
                return convert_to!(DispatchEvent::InviteCreate, message_as_string)
                    .map(Event::Dispatch)
            }
            DispatchEventType::InviteDelete => {
                return convert_to!(DispatchEvent::InviteDelete, message_as_string)
                    .map(Event::Dispatch)
            }
            ...
            DispatchEventType::LastMessages => {
                return convert_to!(DispatchEvent::LastMessages, message_as_string)
                    .map(Event::Dispatch)
            }
            ...
            DispatchEventType::TypingStart => {
                return convert_to!(DispatchEvent::TypingStart, message_as_string)
                    .map(Event::Dispatch)
            }
            DispatchEventType::UserUpdate => {
                return convert_to!(DispatchEvent::UserUpdate, message_as_string)
                    .map(Event::Dispatch)
            }
