...
impl Config {
    pub async fn init(db: &AnyPool) -> Result<Self, Error> {
...
impl ConfigEntity {
    pub async fn get_entity_by_key(db: &AnyPool, key: &str) -> Result<Self, Error> {
    ...
    pub async fn collect_entities(db: &AnyPool) -> Result<Vec<Self>, Error> {
