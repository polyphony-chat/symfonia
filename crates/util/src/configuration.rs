...
impl SymfoniaConfiguration {
    ...
    /// Gets a `static` reference to the [SymfoniaConfiguration] for this application.
    ///
    /// **WILL** panic, if the config has not been initialized yet. This function is meant to be
    ...
    /// ```
    ///
    /// and should only be called if it can be ensured that the config has been initialized.
    pub fn get() -> &'static SymfoniaConfiguration {
        CONFIG
            .get()
            .expect("the configuration has not yet been initialized")
    }
