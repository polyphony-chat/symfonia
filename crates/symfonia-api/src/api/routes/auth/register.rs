...
use chorus::types::{APIError, AuthError, RegisterSchema, jwt::generate_token};
use poem::{
	IntoResponse, Request, handler,
	web::{Data, Json},
};
use serde_json::json;
use util::{
...
pub async fn register(
	...
	let token =
		generate_token(&user.id, user.email.clone().unwrap().as_str(), &cfg.security.jwt_secret);
